bob <- function (x, labels = c("Reference", "Focal"), width = 7, height = 7, 
    ...) 
{
    ...
    plot(theta, gdensity[, 1], type = "l", xlab = "theta", ylab = "Density", 
        ylim = c(0, max(gdensity)), lty = 1, col = 1, main = "Trait Distributions")
    for (g in 2:x$ng) {
    ...
    for (i in 1:length(difitems)) {
        ...
        plot(theta, seq(0, ncat - 1, along.with = theta), type = "n", 
            xlab = "theta", ylab = "Item Score", main = paste0("Item True Score Functions - Item ", 
                difselections[i]), ...)
        for (g in 1:x$ng) {
            ...
            if (x$options$model == "GPCM") 
                ...
                1, g], gpar[i, 2:ncat, g])
            lines(theta, sumpp(pp[, i, 1:ncat, g]), lty = g, 
                col = g)
        }
        legend("bottomright", labels, lty = 1:x$ng, col = 1:x$ng, 
            cex = 0.7, bg = "white")
        ...
        pseudo23 <- x$stats[difitems[i], paste("pseudo23.", x$options$pseudo.R2, 
            sep = "")]
        text(min(theta), ncat - 1, substitute(paste("Pr(", chi[12]^2, 
            ",", chi12, ",", R[12]^2, "=", pseudo12, ",", Delta, 
            "(", beta[1], ")=", beta12, sep = "")), adj = c(0, 
            1), cex = 0.8)
        text(min(theta), (ncat - 1) * 0.9, substitute(paste("Pr(", 
            chi[13]^2, ",", chi13, ",", R[13]^2, "=", pseudo13, 
            sep = "")), adj = c(0, 1), cex = 0.8)
        text(min(theta), (ncat - 1) * 0.8, substitute(paste("Pr(", 
            chi[23]^2, ",", chi23, ",", R[23]^2, "=", pseudo23, 
            sep = "")), adj = c(0, 1), cex = 0.8)
        plot(theta, seq(0, ncat - 1, along.with = theta), type = "n", 
            xlab = "theta", ylab = "Item Score", main = "Differences in Item True Score Functions", 
            ...)
        for (g in 2:x$ng) {
            lines(theta, abs(sumpp(pp[, i, 1:ncat, 1]) - sumpp(pp[, 
                i, 1:ncat, g])), lty = g, col = g)
        }
        plot(theta, seq(0, 1, along.with = theta), type = "n", 
            xlab = "theta", ylab = "Probability", main = "Item Response Functions", 
            ...)
        for (g in 1:x$ng) {
            for (k in 1:ncat) {
                lines(theta, pp[, i, k, g], lty = g, cex = 0.1, 
                  col = g)
            }
        }
        for (g in 1:x$ng) {
            text(x$options$minTheta, 0.8 - (g - 1) * par()$cxy[2], 
                paste(round(gpar[i, , g][!is.na(gpar[i, , g])], 
                  2), collapse = ", "), col = g, adj = c(0, 0), 
                cex = 0.8)
            for (k in 2:ncat) {
                if (!is.na(gpar[i, k, g])) 
                  text(gpar[i, k, g], 0, "|", col = g)
            }
        }
        plot(theta, seq(0, ncat - 1, along.with = theta), type = "n", 
            xlab = "theta", ylab = "Size", main = "Impact (Weighted by Density)", 
            ...)
        for (g in 2:x$ng) {
            lines(theta, gdensity[, g] * abs(sumpp(pp[, i, 1:ncat, 
                1]) - sumpp(pp[, i, 1:ncat, g])), lty = g, col = g)
        }
    }
    par(mfrow = c(1, 2))
    plot(theta, seq(0, sum(!is.na(x$ipar)) - x$ni, along = theta), 
        xlab = "theta", ylab = "TCC", type = "n", main = "All Items", 
        ...)
    for (g in 1:x$ng) {
        apar <- rbind(cpar, gpar[, , g])
        lines(theta, tcc(apar[, 1], apar[, -1, drop = F], theta, 
            model = x$options$model), lty = g, col = g)
    }
    legend("bottomright", labels, lty = 1:x$ng, col = 1:x$ng, 
        bg = "white")
    plot(theta, seq(0, sum(!is.na(gpar[, , 1])) - ndif, along = theta), 
        xlab = "theta", ylab = "TCC", type = "n", main = "DIF Items", 
        ...)
    for (g in 1:x$ng) {
        lines(theta, tcc(gpar[, 1, g], matrix(gpar[, -1, g], 
            nrow = ndif), theta, model = x$options$model), lty = g, 
            col = g)
    }
    legend("bottomright", labels, lty = 1:x$ng, col = 1:x$ng, 
        bg = "white")
    ...
    plot(x$calib$theta, difference, type = "n", xlab = "initial theta", 
        ylab = "initial - purified", ...)
    abline(h = 0)
    abline(h = mean(x$calib$theta - x$calib.sparse$theta), lty = 2)
    for (i in 1:x$ng) {
        points(x$calib$theta[x$group == as.numeric(names(table(x$group))[i])], 
            difference[x$group == as.numeric(names(table(x$group))[i])], 
            col = i, pch = i)
    }
